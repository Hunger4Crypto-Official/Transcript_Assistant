...
- **Coverage is 99.32% line+branch** — measured 2026-09-07 over 1,477 tests
  ...
  - The 11 remaining statements, so nobody re-derives them: `archive.py`
    ...
    public path reaches — the matched segment always has text); `cli.py`
    1420 (`speakers enroll` printing store problems on a failed save), 1783
    (an unfiled record carrying a second, non-fallback analysis);
    `desktop/launch.py`'s `__main__` guard is marked `pragma: no cover`.
  - Nothing that matters is blocked by the sandbox. `http_util`, the ASR/LLM
    providers, and the diarization engine are driven by loopback stub
